...
def run_mc(rvs, args):
    ...
    outputfilename = f"ecc_dim={args.dim}_n={args.n}_null={null_distr_label}.csv"
    ...
    tttest = TopoTestOnesample(n=args.n,
                               dim=int_dim,
                               significance_level=args.alpha)

    tttest.fit(rv=rv_true, n_signature=args.n_signature, n_test=args.n_test)
    for rv_alter in rvs:
        ...
        results.append(
            data_row(
                ...
                accpecth0=tt_out[0],
                pvals=tt_out[1],
                threshold=tttest.representation_threshold,
            )
        )
